...
if(@$_GET['act']=='vote'){
    ...
    if($count>0){
        ...
        //echo "<script> alert('SORRY!! You have already voted this position!');window.location.href='mmuso_president'  </script> ";
    }
    else{
        ...
        if(mysqli_query($conn,$sql)){
            ...
            //echo "<script> alert('Vote successfully casted!,,');window.location.href='mmuso_president'  </script> ";
        }
        else{
...
<?php require_once('include/header1.php')?>
        ...
        <div class="panel-body">
            ...
            <?php
            $check=$conn->query("SELECT reg_number,position_id FROM sec_votes WHERE position_id='$reside' AND reg_number='$reg' ");
            $c=$check->num_rows;
            if($c==1){
        ...
        FROM
            ...
            INNER JOIN smartvoter.schools
                ON (departments.school_id = schools.school_id)WHERE positions.position_id=1";
