...
if(@$_GET['act']=='vote'){
    ...
    $count=$qr->num_rows;
    if($count>0){
        //echo "<script> alert('NYANGAU HII');window.location.href='non_resident_female'  </script> ";
        ...
        if(mysqli_query($conn,$sql)){
            //echo "<script> alert('IKO NDAAAAANI.....');window.location.href='non_resident_female'  </script> ";
...
?>
        ...
        <header class="panel-heading">
            <?php echo $reside_name?> Female Representatatives
        ...
        <div class="panel-body">
            ...
            INNER JOIN smartvoter.schools
                ...
                WHERE positions.position_id='16' AND
                students.gender='F' AND students.residence_id IN (15,16)";
                        $q = mysqli_query($conn, $s);
                        while ($r = mysqli_fetch_assoc($q)) {
                            ?>
                            <tr>
                                <td><img src="<?php echo "../uploads/" . $r['passport']; ?>"
                                         alt="<?php echo $r['passport'] ?>" class="img-rounded"
                                ...
                                <td class="center hidden-phone">
                                    <a class="btn btn-sm btn-success" name="vote"
                                       onclick="return confirm('Confirm your vote')"
                                       href="?action=transview&id=<?php echo $r['candidate_id'] ?>&act=vote">VOTE
                                        NOW </a>
                                </td>
