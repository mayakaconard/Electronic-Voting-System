...
if(@$_GET['act']=='vote'){
    ...
    $count=$qr->num_rows;
    if($count>0){
        //echo "<script> alert('SORRY!! You have already voted this position!');window.location.href='male_representatives'  </script> ";
      $msg= "<div class='alert alert-warning'> SORRY!! You have already voted.</div>";
        ...
        if(mysqli_query($conn,$sql)){
            //echo "<script> alert('IKO NDAAAAANI.....');window.location.href='male_representatives'  </script> ";
...
<?php require_once('include/header1.php')?>
        ...
        <header class="panel-heading">
            <?php echo $sch_name?> Male Representatives
        ...
        FROM
            ...
            INNER JOIN smartvoter.schools
                ON (departments.school_id = schools.school_id)WHERE positions.position_id=8 AND students.school_id='$school';";
